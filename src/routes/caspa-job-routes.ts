...
import { getUserJob, jobProvenance, jobSummary, listUserJobs } from '../services/jobQueueService';
...
router.get('/:id/result', (req, res) => {
  ...
  if (!result.project && job.type === 'commission') {
    ...
    const sibling = listUserJobs(user.id, 100).find((candidate) => {
      const brief = 'input' in candidate ? (candidate as { input?: { brief?: unknown } }).input?.brief : undefined;
    ...
    const siblingBrief = sibling && 'input' in sibling ? (sibling as { input?: { brief?: unknown } }).input?.brief : undefined;
    if (siblingBrief) result.project = siblingBrief;
  }
