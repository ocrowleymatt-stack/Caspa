/**
 * Structured prize draft pipeline: plan → draft → critic → rewrite.
 * Harvested from caspa-studio structuredPipeline.
 */
...
export function parseStructuredPlan(text: string, fallback: Partial<StructuredPlan> = {}): StructuredPlan {
  ...
  return {
    ...
    formatDecision: pickString(parsed.formatDecision, fallback.formatDecision ?? 'novel chapter'),
