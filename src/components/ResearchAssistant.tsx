...
export default function ResearchAssistant({ project, research, chapters, onAddResearch, onAddChapter, onNotify }: Props) {
  ...
  const generateArchitecture = async () => {
    setLoading(true);
    try {
      const data = await AIService.generateArchitecture(project.title, project.genre, project.premise);
      setStructure(data);
  ...
  const handleDeepResearch = async (topic: string) => {
    ...
    try {
      const context = `Non-fiction focus for project ${project.title}. Premise: ${project.premise}`;
  ...
  const synthesize = async () => {
    ...
    try {
      ...
      const prompt = `Synthesize the following research notes into a coherent, granular section for a non-fiction book. 
        Focus on "pieteful" detail: descriptions, sensory facts (smells, sounds, small mechanical or social details), and verified information.
    ...
    <div className="max-w-6xl mx-auto py-6 md:py-12 px-4 md:px-6">
      <header className="mb-8 md:mb-12">
        <div className="flex items-center gap-2 px-3 py-1 bg-blue-50 text-blue-600 rounded-full w-fit mb-4">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest">Research Assistant Mode</span>
        </div>
      ...
      <div className="flex gap-2 md:gap-4 mb-8 p-1 bg-slate-100 rounded-2xl w-full md:w-fit overflow-x-auto no-scrollbar">
        {(['architecture', 'exploration', 'synthesis'] as const).map(step => (
          <button
            key={step}
            onClick={() => setActiveStep(step)}
            className={`px-4 md:px-6 py-2 rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap flex-1 md:flex-none ${
              activeStep === step ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            {step}
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <AnimatePresence mode="wait">
              ...
              >
                {!structure ? (
                  <div className="bg-white p-6 md:p-12 rounded-[2rem] md:rounded-[3rem] border border-slate-100 shadow-sm text-center">
                    <div className="w-12 h-12 md:w-16 md:h-16 bg-blue-50 text-blue-600 rounded-3xl flex items-center justify-center mx-auto mb-4 md:mb-6">
                      <ListTree size={28} />
                    </div>
                    <h3 className="text-lg md:text-xl font-black text-slate-900 mb-2 italic font-serif">Map the Knowledge Graph</h3>
                    <p className="text-xs md:text-sm text-slate-500 max-w-md mx-auto mb-6 md:mb-8">
                      We'll analyze your premise and generate a research roadmap and structural outline tailored for non-fiction excellence.
                    </p>
                    <button 
                      onClick={generateArchitecture}
                      disabled={loading}
                      className="w-full md:w-auto px-8 py-3 md:py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center justify-center gap-3 mx-auto"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100">
                      <h4 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-6 flex items-center gap-2">
                        <Database size={14} /> Structural Chapters
                      </h4>
                      <div className="space-y-4">
                        {structure.chapters?.map((ch: any, idx: number) => (
                          <div key={idx} className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                            <div className="text-[10px] font-black text-blue-600 mb-1">SECTION {idx + 1}</div>
                            <div className="text-sm font-black text-slate-900 italic font-serif mb-1">{ch.title}</div>
                            <p className="text-[10px] text-slate-500 leading-relaxed">{ch.focus}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] flex flex-col h-full">
                          ...
                          return (
                            <button 
                              ...
                              className={`w-full text-left p-4 rounded-2xl border transition-all group relative overflow-hidden ${
                                isSelected 
                                  ? 'bg-blue-600/20 border-blue-500 shadow-[0_0_20px_rgba(37,99,235,0.1)]' 
                                  : 'bg-white/5 border-white/10 hover:bg-white/10'
                              }`}
                            >
                              <div className="flex items-center justify-between mb-1">
                                <div className="flex items-center gap-2">
                                  <div className={`w-1.5 h-1.5 rounded-full transition-all ${isSelected ? 'bg-blue-400 scale-150' : 'bg-white/20'}`} />
                                  <span className={`text-xs font-bold transition-colors ${isSelected ? 'text-blue-200' : ''}`}>{track.topic}</span>
                                </div>
                                {isResearching ? (
                                  <RefreshCcw size={14} className="animate-spin text-blue-400" />
                                ) : (
                                  <ChevronRight size={14} className={`transition-all ${isSelected ? 'opacity-100 text-blue-400' : 'opacity-0 group-hover:opacity-100'}`} />
                                )}
                              </div>
                              <div className="flex items-center gap-2">
                                <span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase ${track.priority === 'High' ? 'bg-amber-500/20 text-amber-500' : 'bg-blue-500/20 text-blue-500'}`}>
                                  {isResearching ? 'Processing Search...' : `${track.priority} Priority`}
                                </span>
                              </div>
                              {isResearching && (
                                <motion.div 
                      ...
                      {selectedTracks.length > 0 && (
                        ...
                        >
                          <button 
                            ...
                            className="w-full py-4 bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-xl shadow-blue-500/20 flex items-center justify-center gap-2"
                          >
                            <Search size={14} />
                            Deep Research Selected ({selectedTracks.length})
                          </button>
                        </motion.div>
                      )}
                    </div>
            ...
            {activeStep === 'exploration' && (
              <motion.div 
                ...
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: 20 }}
                className="bg-white p-8 rounded-[3rem] border border-slate-100 space-y-6"
              >
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-black italic font-serif text-slate-900">Knowledge Clusters</h3>
                  <div className="flex gap-4">
                    ...
                    <button 
                      ...
                      disabled={loading}
                      className="text-[10px] font-black text-blue-600 uppercase tracking-widest flex items-center gap-1 hover:text-blue-700 disabled:opacity-50"
                    >
                      <RefreshCcw size={12} className={loading ? 'animate-spin' : ''} /> Analyze Manuscript Gaps
                    </button>
                  </div>
                </div>
                    ...
                    return (
                      <button 
                        ...
                        className={`p-6 rounded-2xl border text-left transition-all ${
                          isSelected 
                            ? 'border-blue-600 bg-blue-50/50 ring-2 ring-blue-500 shadow-xl shadow-blue-500/10' 
                            : 'border-slate-100 bg-slate-50 hover:border-slate-200'
                        }`}
                      >
                        <div className="flex items-center gap-2 mb-3">
                          <div className={`w-2 h-2 rounded-full ${note.isDeepResearch ? 'bg-blue-500' : 'bg-slate-300'} ${isSelected ? 'scale-125' : ''}`} />
                          <h4 className="text-sm font-black text-slate-900 truncate flex-1">{note.title}</h4>
                          {isSelected && <Sparkles size={12} className="text-blue-600 animate-pulse" />}
                        </div>
                        <p className="text-[10px] text-slate-500 line-clamp-2 leading-relaxed mb-4 font-medium">{note.content}</p>
                ...
                <div className="pt-6 border-t border-slate-50 flex justify-end">
                   <button 
                    ...
                    className="px-8 py-3 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all flex items-center gap-2 shadow-lg disabled:opacity-50"
                   >
                     Synthesize Selected ({selectedNotes.length}) <ArrowRight size={14} />
                   </button>
                </div>
              </motion.div>
            )}
              ...
              >
                <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm min-h-[400px]">
                  <header className="flex items-center justify-between mb-8 pb-4 border-b border-slate-50">
                    <h3 className="text-xl font-black italic font-serif text-slate-900">Synthesized Knowledge Draft</h3>
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            await onAddChapter({
                              ...
                              summary: 'Synthesized from deep research.',
                              order: chapters.length + 1,
                              plotNodeIds: [],
                              tags: ['synthesis'],
                              updatedAt: Date.now()
                            });
                            onNotify('Synthesis added to Manuscript Writing Studio.', 'success');
                          } catch (e) {
                            onNotify('Failed to add synthesis to manuscript.', 'error');
                          } finally {
                            setPushing(false);
                          }
                        }}
                        disabled={pushing || !synthesisResult}
                        className="px-4 py-2 bg-emerald-50 text-emerald-600 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-emerald-100 transition-all flex items-center gap-2 disabled:opacity-50"
                       >
                  ...
                  <div className="prose prose-slate max-w-none prose-p:leading-loose prose-headings:font-serif">
                   <Markdown>{synthesisResult || 'No synthesis generated yet. Select notes in "Exploration" tab.'}</Markdown>
                  </div>
                </div>
              </motion.div>
            )}
        ...
        <div className="lg:col-span-4 space-y-6">
           <div className="bg-blue-900 text-white p-8 rounded-[2.5rem] shadow-xl relative overflow-hidden">
             ...
             <h4 className="text-xs font-black text-blue-400 uppercase tracking-widest mb-4">Prose Intelligence</h4>
             <p className="text-xs font-medium leading-relaxed mb-6">
               Non-fiction succeeds on <b>granularity</b>. Use the Deep Search tool to find exact specifications, historical dates, and sensory artifacts that build authority.
             </p>
             <div className="space-y-4">
                <div className="flex items-start gap-3">
                   <div className="p-2 bg-blue-800 rounded-xl"><Layers size={14} /></div>
                   <div>
                     <p className="text-[10px] font-black uppercase tracking-tight text-blue-200">Layered Research</p>
                     <p className="text-[9px] text-blue-300 font-medium">Combine technical data with eyewitness accounts.</p>
                   </div>
                </div>
                <div className="flex items-start gap-3">
                   <div className="p-2 bg-blue-800 rounded-xl"><Info size={14} /></div>
                   <div>
                     <p className="text-[10px] font-black uppercase tracking-tight text-blue-200">Verification Logic</p>
                     <p className="text-[9px] text-blue-300 font-medium">Auto-crosscheck against verified Google Search indices.</p>
                   </div>
                </div>
             </div>
           </div>
           ...
           <div className="p-6 bg-slate-50 border border-slate-100 rounded-[2.5rem]">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4 px-2">Knowledge Pulse</h4>
              <div className="space-y-2">
                 <div className="flex items-center justify-between p-3 bg-white rounded-xl text-[9px] font-bold">
                    <span className="text-slate-500">Research Notes</span>
                    <span className="text-slate-900">{research.length}</span>
                 </div>
                 <div className="flex items-center justify-between p-3 bg-white rounded-xl text-[9px] font-bold">
                    <span className="text-slate-500">Deep Searches</span>
                    <span className="text-slate-900">{research.filter(r => r.isDeepResearch).length}</span>
                 </div>
                 <div className="flex items-center justify-between p-3 bg-white rounded-xl text-[9px] font-bold">
                    <span className="text-slate-500">Synthesis Events</span>
