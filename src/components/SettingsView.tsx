...
export default function SettingsView({ 
  ...
  return (
    <div className="h-full overflow-y-auto overscroll-contain custom-scrollbar px-2 pb-16" style={{ minHeight: 0 }}>
      <div className="max-w-4xl mx-auto space-y-3 py-2">
      <header className="ethereal-panel p-4 rounded border border-border-subtle shadow-2xl relative overflow-hidden group">
        ...
        <div className="flex items-center gap-2 mb-1.5 relative z-10">
          <div className="p-3 bg-brand-primary/10 rounded text-brand-primary shadow-inner">
            <Settings size={24} />
          </div>
          <h2 className="text-[11px] font-medium font-semibold tracking-tighter text-text-primary italic font-serif">Project Controls</h2>
        </div>
        <p className="text-text-secondary font-semibold uppercase tracking-widest text-xs opacity-40 relative z-10 leading-relaxed">Configure the core parameters and narrative DNA of your archive.</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
        ...
        <section className="space-y-3">
          <div className="flex items-center gap-1.5 text-brand-primary mb-2">
            ...
            <span className="text-xs font-semibold uppercase tracking-widest">Metadata Protocols</span>
          </div>
          
          <div className="space-y-2">
            <div className="p-4 ethereal-panel border border-border-subtle rounded group hover:border-brand-primary/30 transition-all shadow-sm">
              <label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-1.5 opacity-40">Project Title</label>
              <input 
                type="text"
                value={localTitle}
                onChange={(e) => setLocalTitle(e.target.value)}
                className="w-full bg-surface-muted border border-border-subtle rounded px-2 py-2 text-text-primary focus:ring-2 focus:ring-brand-primary/10 focus:border-brand-primary outline-none transition-all font-serif italic text-lg"
              />
            </div>

            <div className="p-4 ethereal-panel border border-border-subtle rounded group hover:border-brand-primary/30 transition-all shadow-sm">
              <label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-1.5 opacity-40">Narrative Format</label>
              <div className="relative">
                <select 
                  value={project.type}
                  onChange={(e) => updateProject({ type: e.target.value as ProjectType })}
                  className="w-full bg-surface-muted/50 border border-border-subtle rounded px-2 py-2 text-text-primary focus:ring-4 focus:ring-brand-primary/5 focus:border-brand-primary outline-none transition-all font-semibold uppercase tracking-widest text-xs appearance-none cursor-pointer pr-12"
                >
                  {PROJECT_TYPES.map(t => (
            ...
            <div className="p-4 ethereal-panel border border-border-subtle rounded group hover:border-brand-primary/30 transition-all shadow-sm">
              <label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-1.5 opacity-40">Narrative Premise</label>
              <textarea 
                value={localPremise}
                onChange={(e) => setLocalPremise(e.target.value)}
                rows={5}
                className="w-full bg-surface-muted border border-border-subtle rounded px-2 py-2 text-text-primary focus:ring-2 focus:ring-brand-primary/10 focus:border-brand-primary outline-none transition-all font-serif italic text-[11px] font-medium leading-relaxed resize-none"
        ...
        <section className="space-y-3">
          <div className="flex items-center gap-1.5 text-brand-primary mb-2">
            <div className="w-1.5 h-1.5 rounded-full bg-brand-primary animate-pulse" />
            <span className="text-xs font-semibold uppercase tracking-widest">Stylistic DNA</span>
          </div>

          <div className="ethereal-panel border border-border-subtle rounded p-4 space-y-3 shadow-xl">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <label className="text-xs font-semibold text-text-secondary uppercase tracking-wider opacity-40 flex items-center gap-2">
                  <Fingerprint size={14} className="text-brand-primary" />
                  Prose Intensity
                </label>
                <span className="text-xs font-semibold text-brand-primary tabular-nums">{(project.styleDNA?.proseIntensity || 50)}%</span>
              </div>
              <input 
                type="range"
                min="0"
              ...
              />
              <div className="flex justify-between text-[10px] font-semibold uppercase tracking-widest text-text-secondary opacity-30">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-semibold text-brand-primary tabular-nums">{(project.styleDNA?.dialogueWeight || 50)}%</span>
              </div>
              <input 
                type="range"
                min="0"
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-semibold text-brand-primary tabular-nums">{(project.styleDNA?.narrativeRhythm || 50)}%</span>
              </div>
              <input 
                type="range"
                min="0"
            ...
            <div className="grid grid-cols-2 gap-2 pt-4">
              <div className="space-y-3">
                <label className="block text-[10px] font-semibold text-text-secondary uppercase tracking-wider opacity-40">Sensory Map</label>
                <div className="relative">
                  <select 
                    value={project.styleDNA?.sensoryPriority || 'balanced'}
                    onChange={(e) => handleStyleUpdate({ sensoryPriority: e.target.value as any })}
                    className="w-full bg-surface-muted border border-border-subtle rounded px-2 py-2 text-xs font-semibold uppercase tracking-widest text-text-primary outline-none appearance-none cursor-pointer"
                  >
              ...
              <div className="space-y-3">
                <label className="block text-[10px] font-semibold text-text-secondary uppercase tracking-wider opacity-40">Lexical Level</label>
                <div className="relative">
                  <select 
          ...
          <div className="p-4 bg-brand-dark border border-border-subtle rounded group transition-all shadow-sm space-y-2">
            <div className="flex items-center gap-1.5 mb-2">
              <Ghost size={18} className="text-brand-primary" />
              <label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider opacity-40">Intelligence Persona</label>
            </div>
            <input 
              type="text"
              ...
              className="w-full ethereal-panel border border-border-subtle rounded px-2 py-2 text-text-primary focus:ring-2 focus:ring-brand-primary/10 focus:border-brand-primary outline-none transition-all font-semibold uppercase tracking-widest text-xs"
            />
            <p className="text-[10px] font-semibold text-text-secondary uppercase tracking-widest opacity-20">Sets the ghostwriter behavior across the entire manuscript.</p>
          </div>
        </section>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 pt-12">
        ...
        <section className="space-y-3">
           <div className="flex items-center gap-1.5 text-brand-primary mb-2">
            <div className="w-1.5 h-1.5 rounded-full bg-brand-primary animate-pulse" />
            <span className="text-xs font-semibold uppercase tracking-widest">Core Protocol</span>
          </div>
          <div className="space-y-1.5">
             <div className="p-4 ethereal-panel border border-border-subtle rounded flex items-center justify-between group">
                <div className="flex flex-col gap-1">
                  <span className="text-xs font-semibold text-text-primary uppercase tracking-widest">Visibility Protocol</span>
                  <span className="text-[10px] font-semibold text-text-secondary uppercase tracking-widest opacity-40">{project.isPublic ? 'Broadcasting to Network' : 'Classified/Offline'}</span>
                </div>
                <button 
              ...
              <div className="p-4 ethereal-panel border border-border-subtle rounded group shadow-sm transition-all hover:border-brand-primary/20">
                <label className="block text-xs font-semibold text-text-secondary uppercase tracking-wider mb-1.5 opacity-40">Narrative Tone Archetype</label>
                <div className="relative">
                  <select 
                    value={project.tone}
            ...
            <div className="p-4 ethereal-panel border border-border-subtle rounded group shadow-xl">
              <label className="block text-xs font-semibold text-text-secondary uppercase tracking-widest mb-3 opacity-40">Narrative Boundary (Target Word Count)</label>
              <div className="relative group/input">
                <input 
                  type="number"
                  value={project.targetWordCount || ''}
                  onChange={(e) => updateProject({ targetWordCount: parseInt(e.target.value) || 0 })}
                  className="w-full bg-surface-muted/50 border border-border-subtle rounded px-2 py-2 text-text-primary focus:ring-4 focus:ring-brand-primary/5 focus:border-brand-primary outline-none transition-all font-semibold italic text-[11px] font-medium tabular-nums pr-24"
                  placeholder="50000"
                />
                <div className="absolute right-8 top-1/2 -translate-y-1/2 flex flex-col items-end">
                  <span className="text-xs font-semibold text-brand-primary uppercase tracking-[0.2em]">Words</span>
                  <span className="text-[10px] font-semibold text-text-secondary uppercase tracking-[0.2em] opacity-40">Capacity</span>
                </div>
              </div>
              <p className="mt-6 text-[10px] font-semibold text-text-secondary uppercase tracking-widest opacity-20 leading-relaxed">
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between mb-3 px-2">
            <div className="flex items-center gap-1.5 text-brand-primary">
              <div className="w-1.5 h-1.5 rounded-full bg-brand-primary animate-pulse" />
              <span className="text-xs font-semibold uppercase tracking-widest">Intelligence Ecosystem</span>
            </div>
            <div className="text-[10px] font-semibold text-brand-accent uppercase tracking-[0.2em] opacity-80 px-3 py-1 rounded-full border border-brand-accent/20 bg-brand-accent/10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 pb-8">
            {[
              ...
              <button
                ...
                className={`p-4 rounded border transition-all group/opt active:scale-[0.97] flex flex-col items-center justify-center gap-2 relative overflow-hidden ${
              ...
              >
                ...
                <div className="text-xs font-semibold uppercase tracking-[0.2em] relative z-10">{provider.label}</div>
                <div className={`text-[10px] font-medium uppercase tracking-widest opacity-40 relative z-10 ${
            ...
            <div className="p-4 ethereal-panel border border-border-subtle rounded space-y-3 shadow-2xl relative overflow-hidden group">
              <div className="absolute inset-0 bg-brand-primary opacity-[0.01] pointer-events-none" />
              <div className="space-y-1.5 relative z-10">
                <h3 className="text-xs font-semibold text-brand-primary uppercase tracking-wider flex items-center gap-2">
              ...
              <div className="flex flex-col gap-2 relative z-10">
                <button 
                  ...
                  className="w-full py-1 bg-brand-primary text-white rounded font-semibold text-xs uppercase tracking-widest hover:brightness-110 active:scale-[0.98] transition-all shadow-xl shadow-brand-primary/30 border border-white/10"
                >
                  Global Account Scan
                </button>
                ...
                <button 
                  ...
                  className="w-full py-2 bg-white/5 border border-white/5 text-text-primary rounded font-semibold text-xs uppercase tracking-[0.2em] hover:bg-white/10 transition-all opacity-80 hover:opacity-100 flex items-center justify-center gap-2"
                >
            ...
            <div className="p-4 bg-brand-dark border border-brand-primary/20 rounded space-y-2 shadow-2xl relative overflow-hidden group">
              ...
              <div className="flex items-center gap-2 text-brand-primary relative z-10">
                ...
                <h3 className="text-xs font-semibold uppercase tracking-widest">Distributed Fallback active</h3>
              </div>
              <div className="space-y-2 relative z-10">
                {[
                  ...
                  <div key={item.key} className="flex items-center justify-between px-2 py-2 ethereal-panel/50 rounded border border-border-subtle group/key hover:border-brand-primary/20 transition-colors">
                    <code className="text-[10px] font-mono font-semibold text-brand-primary opacity-40 uppercase group-hover/key:opacity-100">{item.key}</code>
        ...
        <section className="space-y-3">
          <div className="flex items-center gap-1.5 text-brand-primary mb-3 px-2">
            <div className="w-1.5 h-1.5 rounded-full bg-brand-primary animate-pulse" />
            <span className="text-xs font-semibold uppercase tracking-widest">Identity & Sync Diagnostics</span>
          </div>

          <div className="ethereal-panel border border-border-subtle rounded p-4 shadow-xl space-y-2">
            <p className="text-xs text-text-secondary leading-relaxed font-medium italic opacity-60">
              If your books or manuscripts are missing, verify your synchronization identity below.
            </p>
            
            <div className="space-y-1.5">
              <div className="bg-surface-muted/50 p-4 rounded border border-border-subtle flex flex-col gap-1.5">
                <span className="text-[10px] font-semibold uppercase tracking-wider text-text-secondary opacity-40 flex items-center gap-2">
                  <Shield size={12} />
                  Direct Auth Signature
                </span>
                <span className="text-md font-mono font-medium text-text-primary break-all selection:bg-brand-primary/30">
                  {localStorage.getItem('currentUserEmail') || 'Authenticating...'}
                </span>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                <div className="bg-brand-primary/5 p-4 rounded border border-brand-primary/10 flex flex-col gap-2">
                  <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-brand-primary">Caspa Variant check</span>
                  <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-brand-primary animate-pulse" />
                    <span className="text-xs font-semibold text-text-primary uppercase tracking-widest">Case-Insensitive active</span>
                  </div>
                </div>
                <div className="bg-brand-primary/5 p-4 rounded border border-brand-primary/10 flex flex-col gap-2">
                  <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-brand-primary">Gmail Logic check</span>
                  <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-brand-primary animate-pulse" />
                    <span className="text-xs font-semibold text-text-primary uppercase tracking-widest">Dot-ignoring active</span>
                  </div>
                </div>
              </div>
            </div>

            <p className="text-[10px] font-semibold uppercase tracking-widest text-text-secondary/40 pt-4 border-t border-white/5">
        ...
        <section className="space-y-3">
          <div className="flex items-center gap-1.5 text-brand-primary mb-3 px-2">
            <div className="w-1.5 h-1.5 rounded-full bg-brand-primary animate-pulse" />
            <span className="text-xs font-semibold uppercase tracking-widest">Compliance & Resilience</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            <div className="p-4 ethereal-panel border border-border-subtle rounded flex items-center justify-between group">
              <div className="flex flex-col gap-1">
                <span className="text-xs font-semibold text-text-primary uppercase tracking-widest flex items-center gap-2">
                  <Lock size={12} className={project.isLocked ? 'text-brand-primary' : 'text-text-secondary'} />
                  Write-Lock Protocol
                </span>
                <span className="text-[10px] font-semibold text-text-secondary uppercase tracking-widest opacity-40">{project.isLocked ? 'Immutable State' : 'Operational/Mutable'}</span>
              </div>
              <button 
            ...
            <button 
              ...
              className="p-4 ethereal-panel border border-border-subtle rounded flex items-center justify-between group hover:border-brand-primary/50 transition-all text-left shadow-lg overflow-hidden relative"
            >
              ...
              <div className="flex flex-col gap-1 relative z-10">
                ...
                <span className="text-[10px] font-semibold text-text-secondary uppercase tracking-widest opacity-40">Download encrypted project JSON blueprint</span>
              </div>
        ...
        <div className="flex items-center gap-1.5 text-brand-primary mb-3 px-2">
          <div className="w-1.5 h-1.5 rounded-full bg-brand-primary animate-pulse" />
          <span className="text-xs font-semibold uppercase tracking-widest">Compliance & Narrative Depth</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {MATURITY_LEVELS.map((level) => (
            <button
              ...
              className={`p-4 rounded border text-left transition-all group relative overflow-hidden active:scale-95 ${
            ...
            >
              <div className={`w-10 h-10 rounded flex items-center justify-center mb-1.5 shadow-inner transition-all duration-500 ${
              ...
              <div className="font-semibold mb-1 uppercase tracking-[0.2em] text-[10px]">{level.label}</div>
      ...
      <section className="pt-20 border-t border-border-subtle">
        <div className={`rounded p-4 border transition-all duration-700 relative overflow-hidden group ${
          isConfirmingDelete ? 'bg-red-500/10 border-red-500/40 shadow-2xl' : 'bg-red-500/5 border-red-500/20'
        }`}>
          <div className="absolute top-0 right-0 p-4 opacity-[0.03] group-hover:opacity-10 transition-opacity duration-1000">
             <Trash2 size={160} className="text-red-500" />
          </div>
          <div className="flex flex-col md:flex-row items-center gap-2 relative z-10">
            <div className={`p-4 text-red-500 rounded shadow-inner border border-red-500/20 transition-transform duration-500 ${isConfirmingDelete ? 'rotate-12 scale-110' : ''}`}>
              <Flame size={32} />
            </div>
            <div className="flex-1 text-center md:text-left">
              <h3 className="text-[11px] font-medium font-semibold text-text-primary italic font-serif tracking-tight mb-2">Destruct Sequence</h3>
              <p className="text-[11px] text-text-secondary leading-relaxed mb-3 font-medium italic opacity-60">
              ...
              <AnimatePresence mode="wait">
                {!isConfirmingDelete ? (
                  <motion.button 
                    ...
                    className="px-2 py-1 bg-red-500 hover:bg-red-600 text-white font-semibold text-xs uppercase tracking-widest rounded transition-all shadow-2xl shadow-red-500/20 active:scale-95"
                  >
                  ...
                  >
                    <div className="space-y-1.5">
                      <p className="text-xs font-semibold text-red-500 uppercase tracking-widest bg-red-500/5 py-2 px-2 rounded border border-red-500/20">
                      ...
                      <input 
                        ...
                        className="w-full ethereal-panel border border-red-500/30 rounded px-2 py-2 text-text-primary focus:ring-2 focus:ring-red-500/20 focus:border-red-500 outline-none transition-all font-semibold uppercase tracking-widest text-xs"
                        autoFocus
                      />
                    </div>
                    <div className="flex items-center gap-2">
                      <button 
                        ...
                        className={`flex-1 py-1 font-semibold text-xs uppercase tracking-widest rounded transition-all shadow-2xl ${
                      ...
                      <button 
                        ...
                        className="px-2 py-1 bg-surface-muted text-text-secondary font-semibold text-xs uppercase tracking-widest rounded border border-border-subtle hover:text-text-primary transition-colors"
                      >
                        Abort
                      </button>
                    </div>
