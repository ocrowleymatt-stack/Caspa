    ...
    <div className="h-full flex flex-col gap-1.5" style={{ minHeight: 0 }}>
      <header className="flex items-center justify-between ethereal-panel p-3 rounded-md border border-border-subtle shadow-2xl relative overflow-hidden group">
        ...
        <div className="flex items-center gap-1.5 relative z-10">
          ...
          <div>
            <h2 className="text-xs font-semibold font-semibold tracking-tight text-text-primary mb-1 italic font-serif">Critic Swarm</h2>
            <p className="text-xs text-text-secondary font-semibold uppercase tracking-widest opacity-40 flex items-center gap-2">
              Multi-agent specialist peer review.
              {localCritiques.length > 0 && <span className="text-brand-primary font-semibold ml-2 animate-pulse">• {localCritiques.length} PERSISTED REPORTS</span>}
            </p>
          </div>
        </div>
          ...
          <AnimatePresence mode="popLayout">
            {sortedCritiques.length > 0 ? sortedCritiques.map((c, i) => (
              <motion.div 
                ...
                layout
                className={`ethereal-panel border rounded-md p-2 shadow-sm hover:shadow-2xl transition-all relative overflow-hidden group ${
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        <h4 className="font-semibold text-text-primary text-[11px] uppercase tracking-widest">{c.agentName}</h4>
                        {c.timestamp && (
                          <span className="text-xs font-semibold text-text-secondary opacity-30 italic">
                            {new Date(c.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-1.5 mt-1.5">
                        <span className={`text-[10px] font-semibold px-3 py-1 rounded-full uppercase tracking-widest shadow-sm ${
                        ...
                        <span className="text-[10px] font-semibold text-text-secondary uppercase tracking-[0.2em] opacity-40">{c.role} layer</span>
                      </div>
                    </div>
                  </div>
                </div>

                <p className="text-text-primary text-lg leading-relaxed mb-3 font-serif italic border-l-4 border-brand-primary/20 pl-8 py-2 relative z-10">
                  "{c.content}"
                </p>

                <div className="space-y-1.5 bg-brand-dark/40 p-3 rounded-md border border-border-subtle relative z-10">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[10px] font-semibold text-text-secondary uppercase tracking-wider opacity-50 flex items-center gap-2">
                      <Target size={14} className="text-brand-primary" />
                      Corrective Directives
                    </p>
                    <div className="text-[10px] font-semibold text-brand-primary/40 uppercase tracking-widest">Protocol Sync: Enabled</div>
                  </div>
                  {c.suggestions.map((s, idx) => (
                    <div key={idx} className="flex gap-2 items-start p-4 rounded ethereal-panel border border-transparent hover:border-border-subtle group/item transition-all">
                      <div className={`w-8 h-8 rounded-md flex items-center justify-center flex-shrink-0 text-xs font-semibold transition-all shadow-lg ${
                      ...
                      <div className="flex-1">
                        <p className={`text-[11px] font-medium leading-relaxed transition-all italic font-serif ${
                          s.accepted ? 'text-text-primary font-medium' : 
              ...
              <div className="h-full flex flex-col items-center justify-center text-center p-20 border-2 border-dashed border-border-subtle rounded text-text-secondary ethereal-panel shadow-inner relative group overflow-hidden">
        ...
        <div className="bg-brand-dark text-text-primary rounded-md p-3 flex flex-col gap-1.5 shadow-[0_50px_100px_rgba(0,0,0,0.5)] border border-border-subtle relative overflow-y-auto overscroll-contain custom-scrollbar group">
           ...
           <header className="relative">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-xs font-semibold font-semibold italic font-serif tracking-tight">Critical Intelligence</h3>
              </div>
              <p className="text-xs text-text-secondary font-semibold uppercase tracking-widest opacity-40">Active Multi-Agent Sync</p>
           </header>
           ...
           <div className="space-y-3 relative flex-1">
              {chapters.some(c => (c.directives || []).length > 0) && (
                <motion.div 
                  ...
                  className="p-2 btn-nexus-primary rounded-md shadow-2xl shadow-brand-primary/20 relative overflow-hidden group/alert"
                >
                  <div className="absolute top-0 right-0 p-3 opacity-10 group-hover/alert:scale-110 transition-transform">
                    <CheckCircle size={60} />
                  </div>
                  <p className="text-xs text-white/60 font-semibold uppercase tracking-wider mb-1.5 flex items-center gap-2">
                    <CheckCircle size={14} className="fill-white/20" />
                    Logic Patch Buffered
                  </p>
                  <p className="text-[11px] font-medium leading-relaxed mb-3 italic font-serif">
                    Suggestions integrated into manuscript metadata. Run redrafting engine to apply changes.
                  </p>
                  <button 
                    onClick={() => setView('architect')}
                    className="w-full py-1 bg-white text-brand-primary rounded font-semibold text-xs uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-1.5 shadow-xl active:scale-95"
                  >
              ...
              <div className="p-2 ethereal-panel border border-border-subtle rounded-md hover:border-brand-primary/20 transition-colors shadow-lg group/box">
                <p className="text-[10px] font-semibold text-text-secondary uppercase tracking-widest opacity-30 mb-1.5 group-hover/box:opacity-60 transition-opacity">Swarm Consensus</p>
                <p className="text-[11px] font-medium font-medium text-text-primary leading-[1.8] italic font-serif border-l-2 border-brand-primary/10 pl-6 group-hover/box:border-brand-primary transition-all">
              ...
              <div className="space-y-2">
                <p className="text-[10px] font-semibold text-text-secondary uppercase tracking-widest opacity-30 flex items-center gap-1.5 border-b border-border-subtle pb-4">
                  <Activity size={14} />
                ...
                {['Dramaturgical', 'Stylistic', 'Thematic', 'World Logic', 'Marketable'].map(p => (
                   <div key={p} className="flex items-center justify-between group/p cursor-default">
                      <span className="text-xs font-semibold text-text-secondary opacity-50 group-hover/p:text-brand-primary group-hover/p:opacity-100 transition-all uppercase tracking-widest">{p}</span>
           ...
           <footer className="pt-8 border-t border-border-subtle relative">
              <div className="flex items-center justify-between">
                <div className="flex flex-col">
                  <p className="text-[10px] font-semibold text-text-secondary uppercase tracking-widest opacity-20">Protocol</p>
                  <p className="text-xs font-semibold text-text-secondary opacity-50 font-serif italic italic">Iterative Disagreement v5.0</p>
                </div>
                <div className="text-[10px] font-semibold text-brand-primary bg-brand-primary/5 px-3 py-1.5 rounded-md border border-brand-primary/20 shadow-sm">AGENT_SYNC_OK</div>
              </div>
           </footer>
        </div>
      </div>
