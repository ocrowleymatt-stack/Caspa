...
// Chapters 1 and 7 already cleared on Atlas. Resume only the remaining failures.
...
async function ai(prompt) {
  const r = await fetch(`http://127.0.0.1:${PORT}/api/ai/call`, {
    ...
    body: JSON.stringify({prompt,maxTokens:14000,intelligenceMode:'balanced',taskHint:'factual',useWebSearch:true,skipLocalFallback:true}),
...
for(const order of TARGET_ORDERS){
  ...
  // Persist after each chapter so an SSH timeout cannot discard completed work.
  job.checkpoint.chapters=chapters;
  job.checkpoint.phase=`surgical-qa-repair:chapter-${order+1}-cleared`;
