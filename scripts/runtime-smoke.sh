...
try:
    ...
    print("NWP:", nwp["data"]["outputId"], "structured?", "structured" in nwp["data"])

    cont = req("POST", "/api/casper/continue", {
        "projectId": pid,
        "currentText": nwp["data"]["text"][:1200],
        "mode": "continue",
    }, token=token, timeout=180)
    print("CONTINUE:", cont["data"]["outputId"])
    ...
    gold = req("POST", "/api/gold/run", {"projectId": pid, "source": nwp["data"]["text"][:2000]}, token=token, timeout=180)
    print("GOLD:", gold["data"]["outputId"])

    outs = req("GET", f"/api/outputs?projectId={pid}", token=token)
    print("OUTPUTS:", len(outs["data"]))
