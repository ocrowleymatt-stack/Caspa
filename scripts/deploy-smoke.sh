...
# All remaining API routes are deliberately authenticated. Run this smoke on
# loopback and supply the same server-only proxy secret loaded by Caspa.
SMOKE_UID="${CASPA_SMOKE_UID:-deploy-smoke}"
SMOKE_SECRET="${CASPA_SMOKE_PROXY_SECRET:-${CASPA_PROXY_SHARED_SECRET:-}}"
SMOKE_GROUPS="${CASPA_SMOKE_GROUPS:-}"
[[ -n "$SMOKE_SECRET" ]] || fail "CASPA_SMOKE_PROXY_SECRET (or CASPA_PROXY_SHARED_SECRET) is required"
AUTH_HEADERS=(-H "X-Authentik-Uid: $SMOKE_UID" -H "X-Caspa-Proxy-Secret: $SMOKE_SECRET")
if [[ -n "$SMOKE_GROUPS" ]]; then AUTH_HEADERS+=(-H "X-Authentik-Groups: $SMOKE_GROUPS"); fi
...
curl -sf "${AUTH_HEADERS[@]}" "$BASE/api/ollama/smoke" | grep -q '"success":true' || fail "/api/ollama/smoke"
ok "/api/ollama/smoke"

# Gold passes
curl -sf "${AUTH_HEADERS[@]}" "$BASE/api/caspa/gold/passes" | grep -q 'structure' || fail "/api/caspa/gold/passes"
ok "/api/caspa/gold/passes"

# Job audit
curl -sf "${AUTH_HEADERS[@]}" "$BASE/api/caspa/gold/jobs/audit" | grep -q '"activeJobs"' || fail "/api/caspa/gold/jobs/audit"
ok "/api/caspa/gold/jobs/audit"

# Storage backups list
curl -sf "${AUTH_HEADERS[@]}" "$BASE/api/caspa/storage/backups" | grep -q '"success":true' || fail "/api/caspa/storage/backups"
ok "/api/caspa/storage/backups"

# Novel Write Pro quality pass
QP=$(curl -sf "${AUTH_HEADERS[@]}" -X POST "$BASE/api/caspa/novel-write-pro/quality-pass" \
...
RESEARCH=$(curl -sf "${AUTH_HEADERS[@]}" --max-time 70 -X POST "$BASE/api/caspa/research/deep" \
  -H 'Content-Type: application/json' \
...
AICALL=$(curl -sf "${AUTH_HEADERS[@]}" --max-time 90 -X POST "$BASE/api/ai/call" \
  -H 'Content-Type: application/json' \
...
API_MISS_HEADERS=$(curl -sI "${AUTH_HEADERS[@]}" "$BASE/api/this-does-not-exist") || fail "HEAD /api miss"
...
API_MISS_BODY=$(curl -s "${AUTH_HEADERS[@]}" "$BASE/api/this-does-not-exist") || fail "GET /api miss body"
...
API_POST_MISS=$(curl -s "${AUTH_HEADERS[@]}" -X POST "$BASE/api/this-does-not-exist" -H 'Content-Type: application/json' -d '{}') || fail "POST /api miss"
...
AI_GET=$(curl -s "${AUTH_HEADERS[@]}" -o /tmp/caspa-ai-get.body -w '%{content_type}' "$BASE/api/ai/call") || fail "GET /api/ai/call"
