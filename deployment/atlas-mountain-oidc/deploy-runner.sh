...
# Keep the normal deploy implementation untouched on disk, but patch its human
# authentication canary in a private one-run copy. Rollback, archive validation,
# OIDC transport, Device Bridge authentication and deployment locking are all
# still executed by the normal deploy implementation.
...
# Human login is deliberately disabled while the legacy Atlas edge/auth stack is
# repaired. Deployment OIDC and paired-device authentication remain separate.
